...
export function PaymentsPage() {
  ...
  function openPayment(row: MonthlyPayment) { setSelectedPayment(row); paymentForm.setFieldsValue({ amount: row.amount ?? 0, status: row.effectiveStatus === "Paid" ? "Paid" : "Unpaid", paidOn: row.paidOn ?? undefined }); }
  ...
  return <div>
    ...
    <Modal title={`${selectedPayment?.athleteName ?? ""} ödemesi`} open={selectedPayment !== null} okText="Kaydet" cancelText="Vazgeç" confirmLoading={paymentMutation.isPending} onCancel={() => setSelectedPayment(null)} onOk={() => paymentForm.submit()}><Form form={paymentForm} layout="vertical" onFinish={(values) => paymentMutation.mutate(values)}><Form.Item name="amount" label="Toplam tutar" rules={[{ required: true }]}><InputNumber min={0.01} precision={2} className="full-width" /></Form.Item><Form.Item name="status" label="Durum" rules={[{ required: true }]}><Select options={[{ value: "Paid", label: "Ödendi" }, { value: "Unpaid", label: "Ödenmedi" }, { value: "Pending", label: "Bekliyor" }]} /></Form.Item><Form.Item name="paidOn" label="Ödeme tarihi"><Input type="date" /></Form.Item></Form></Modal>
