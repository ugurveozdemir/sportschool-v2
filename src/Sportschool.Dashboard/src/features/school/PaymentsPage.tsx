...
type PaymentForm = { amount: number; amountPaid?: number; status: PaymentStatus; paidOn?: string };
...
export function PaymentsPage() {
  ...
  function openPayment(row: MonthlyPayment) { setSelectedPayment(row); paymentForm.setFieldsValue({ amount: row.amount ?? 0, amountPaid: row.amountPaid ?? 0, status: row.effectiveStatus === "Paid" ? "Paid" : "Unpaid", paidOn: row.paidOn ?? undefined }); }
  ...
  return <div>
    ...
    <Table<MonthlyPayment> rowKey="athleteProfileId" loading={paymentsQuery.isLoading} dataSource={paymentsQuery.data ?? []} pagination={{ pageSize: 12, showSizeChanger: false }} locale={{ emptyText: <Empty description="Bu ay için sporcu bulunmuyor." /> }} columns={[
      ...
      { title: "Tutar", dataIndex: "amount", key: "amount", render: formatCurrency }, { title: "Ödenen", dataIndex: "amountPaid", key: "amountPaid", render: formatCurrency }, { title: "Kalan", dataIndex: "balance", key: "balance", render: formatCurrency },
      { title: "Durum", key: "status", render: (_, row) => statusTag(row.effectiveStatus) },
      { title: "İşlem", key: "action", render: (_, row) => <Button size="small" type="primary" icon={<CheckOutlined />} disabled={!row.amount} onClick={() => openPayment(row)}>Ödeme işle</Button> }
    ]} />
    <Modal title={`${selectedPayment?.athleteName ?? ""} ödemesi`} open={selectedPayment !== null} okText="Kaydet" cancelText="Vazgeç" confirmLoading={paymentMutation.isPending} onCancel={() => setSelectedPayment(null)} onOk={() => paymentForm.submit()}><Form form={paymentForm} layout="vertical" onFinish={(values) => paymentMutation.mutate(values)}><Form.Item name="amount" label="Toplam tutar" rules={[{ required: true }]}><InputNumber min={0.01} precision={2} className="full-width" /></Form.Item><Form.Item name="amountPaid" label="Ödenen tutar"><InputNumber min={0} precision={2} className="full-width" /></Form.Item><Form.Item name="status" label="Durum" rules={[{ required: true }]}><Select options={[{ value: "Paid", label: "Ödendi" }, { value: "Unpaid", label: "Ödenmedi" }, { value: "Pending", label: "Bekliyor" }]} /></Form.Item><Form.Item name="paidOn" label="Ödeme tarihi"><Input type="date" /></Form.Item></Form></Modal>
