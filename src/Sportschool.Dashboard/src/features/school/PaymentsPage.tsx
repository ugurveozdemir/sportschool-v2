...
export function PaymentsPage() {
  ...
  return <div>
    ...
    <Modal title={`${selectedPayment?.athleteName ?? ""} ödemesi`} open={selectedPayment !== null} okText="Kaydet" cancelText="Vazgeç" confirmLoading={paymentMutation.isPending} onCancel={() => setSelectedPayment(null)} onOk={() => paymentForm.submit()}><Form form={paymentForm} layout="vertical" onFinish={(values) => paymentMutation.mutate(values)}><Form.Item name="amount" label="Aylık aidat tutarı" rules={[{ required: true, message: "Tutar zorunludur." }, { type: "number", min: 0.01, message: "Tutar sıfırdan büyük olmalıdır." }]}><InputNumber min={0.01} precision={2} className="full-width" disabled={selectedPayment?.amount !== null} /></Form.Item><Typography.Paragraph type="secondary">{selectedPayment?.amount === null ? "Okul veya sporcu aidatı tanımlı olmadığı için tutarı manuel girin." : "Tutarı değiştirmek için okul ayarını veya sporcuya özel aidatı düzenleyin. Ödendi seçildiğinde tutarın tamamı tahsil edilmiş sayılır."}</Typography.Paragraph><Form.Item name="status" label="Durum" rules={[{ required: true }]}><Select options={[{ value: "Paid", label: "Ödendi" }, { value: "Unpaid", label: "Ödenmedi" }, { value: "Pending", label: "Bekliyor" }]} /></Form.Item><Form.Item name="paidOn" label="Ödeme tarihi"><Input type="date" /></Form.Item></Form></Modal>
