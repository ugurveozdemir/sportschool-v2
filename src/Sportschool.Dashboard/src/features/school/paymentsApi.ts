...
export function savePayment(athleteId: string, year: number, month: number, input: { amount: number; amountPaid: number | null; status: PaymentStatus; paidOn: string | null }): Promise<void> { return apiRequest<void>(`/api/school/athletes/${athleteId}/payments/${year}/${month}`, { method: "PUT", body: input }); }
